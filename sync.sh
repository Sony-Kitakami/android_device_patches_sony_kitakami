...
if ! (grep -Fxq "THIS FILE HAS BEEN PATCHED" ${VOLD_DIR}/Utils.cpp); then
	mv ${VOLD_DIR}/Utils.cpp ${RVRT_VOLD_DIR}/
fi

if ! (grep -Fxq "THIS FILE HAS BEEN PATCHED" ${VOLD_DIR}/Android.mk); then
...
if ! (grep -Fxq "THIS FILE HAS BEEN PATCHED" ${TORCH_DIR}/FlashlightTile.java); then
